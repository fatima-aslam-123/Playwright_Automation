# Jobs Page Test Flow Skill

## Objective

While generating or updating Playwright test cases for the Jobs page, always follow this strict execution pattern.
...
4. Do NOT reload or re-open the Jobs page for every test case.
5. All test cases must run on the **same already opened Jobs page session**.
...
* Perform login
* Navigate to Jobs page (once)
* Apply filter or perform action
...
* Avoid `page.goto()` repeatedly for Jobs page.
* Ensure tests are session-aware, not page-restarting.
* Each test must assume user is already on Jobs page (after first test).
